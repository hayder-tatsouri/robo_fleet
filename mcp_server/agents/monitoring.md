...
- get_fleet_status(robot_ids, timeout): Get position, battery, and online status for all robots. Default checks tb1, tb2, tb3. Specify robot_ids to check custom IDs like "pearlguard1".
...
Guidelines:
- get_fleet_status() without arguments only checks tb1, tb2, tb3. If the user mentions a different robot (e.g. "pguard", "pearlguard1"), use get_robot_position(robot_id) or call get_fleet_status with the specific robot_ids.
